...
- `:TodoGraphView [dir]` opens a modal tree view (runs `generate --format json --output <temp>` to render the tree).
...
- `:TodoGraphView [dir]` — modal tree view (toggle nodes with `<CR>`, refresh with `r`, close with `q`; generates JSON to a temp file).
